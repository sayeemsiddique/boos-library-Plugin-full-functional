...
function mbt_integrate_woo_breadcrumbs($trail) {
	...
	if(is_tax('mbt_author') or is_tax('mbt_genre') or is_tax('mbt_series') or is_tax('mbt_tag') or is_tax('mbt_publisher') || (mbt_is_archive_query() && ($mbt_archive_query->is_tax('mbt_author') or $mbt_archive_query->is_tax('mbt_genre') or $mbt_archive_query->is_tax('mbt_series') or $mbt_archive_query->is_tax('mbt_tag') or $mbt_archive_query->is_tax('mbt_publisher')))) {
...
function mbt_get_breadcrumbs($delimiter = '') {
	...
	} else if(is_tax('mbt_author') or is_tax('mbt_series') or is_tax('mbt_genre') or is_tax('mbt_tag') or is_tax('mbt_publisher')) {
