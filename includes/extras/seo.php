...
function mbt_filter_wpseo_defaults($defaults, $option_name) {
	if($option_name == 'wpseo_titles') {
		...
		$defaults['title-tax-mbt_publisher'] = '%%term_title%% %%sep%% %%sitename%%';
		$defaults['metadesc-tax-mbt_publisher'] = '%%term_description%%';
	}
...
function mbt_add_wpseo_opengraph_image() {
	...
	if(is_tax('mbt_author') or is_tax('mbt_series') or is_tax('mbt_genre') or is_tax('mbt_tag') or is_tax('mbt_publisher')) {
...
function mbt_seo_wp_title($title) {
	...
	if(is_tax('mbt_author') or is_tax('mbt_series') or is_tax('mbt_genre') or is_tax('mbt_tag') or is_tax('mbt_publisher') or (mbt_is_archive_query() and $mbt_archive_query->is_tax())) {
...
function mbt_seo_woo_title($title, $sep, $raw_title) {
	if(is_tax('mbt_author') or is_tax('mbt_series') or is_tax('mbt_genre') or is_tax('mbt_tag') or is_tax('mbt_publisher') or is_post_type_archive('mbt_book') or is_singular('mbt_book') or mbt_is_archive_query()) {
		return $raw_title;
	}
	return $title;
}
