...
function mbt_mybooktable_shortcode($attrs) {
	...
	if(!empty($attrs['list'])) {
		...
		} else if(!empty($attrs['publisher'])) {
			$wp_query = new WP_Query(array('post_type' => 'mbt_book', 'mbt_publisher' => $attrs['publisher'], 'orderby' => 'menu_order', 'posts_per_page' => $max_books));
		} else {
...
function mbt_add_authormedia_shortcodes($shortcodes) {
	...
	$shortcodes['mybooktable'] = array('name' => 'MyBookTable', 'shortcodes' => array(
		...
		'mybooktable-publisher' => array(
			'title'			=> __('All Books with publisher', 'mybooktable'),
			'description'	=> __('List all the books with a given publisher in an embedded book listing.', 'mybooktable'),
			'settings'		=> array(
				'publisher'	=> array(
					'title'			=> __('Publisher', 'mybooktable'),
					'description'	=> '',
					'type'			=> 'dropdown',
					'choices'		=> mbt_get_taxonomy_names('mbt_publisher')
				),
				'gridview'	=> array(
					'title'			=> 'Force Grid View Display?',
					'default'		=> false,
		...
		'mybooktable-list' => array(
			...
			'settings'		=> array(
				'list'	=> array(
					...
					'description'	=> '',
					'type'			=> 'dropdown',
					'choices'		=> array("series" => __("Series", 'mybooktable'), "genres" => __("Genres", 'mybooktable'), "tags" => __("Tags", 'mybooktable'), "publishers" => __("Publishers", 'mybooktable'), "authors" => __("Authors", 'mybooktable'))
				),
				'display'	=> array(
					'title'			=> __('Display Style', 'mybooktable'),
					'description'	=> '',
