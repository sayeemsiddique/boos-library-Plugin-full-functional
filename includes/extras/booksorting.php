...
function mbt_render_sort_books_page() {
	...
	if(!empty($_REQUEST['book_order']) and $_REQUEST['book_order'] == 'attribute') {
		...
		if(!empty($data) and is_array($data)) {
			...
			foreach ($query->posts as $post) {
				...
				$publisher = '';
				$publishers = get_the_terms($post->ID, 'mbt_publisher');
				if(!empty($publishers) and !is_wp_error($publishers)) {
					if(!empty($publishers[0]) and !is_wp_error($publishers[0])) {
	...
	function mbt_add_book_sorting_attribute(attribute) {
		...
		var attributes = {
			...
			'publisher': mbt_admin_pages_i18n.publisher,
