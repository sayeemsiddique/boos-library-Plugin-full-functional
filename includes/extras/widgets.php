...
class MBT_Taxonomies extends WP_Widget {
	function __construct() {
		$widget_ops = array('classname' => 'mbt_taxonomies', 'description' => __("A list of Authors, Genres, Series, Publishers, or Tags.", 'mybooktable'));
	...
	function widget($args, $instance) {
		...
		} else if($tax === 'mbt_publisher') {
			$title = __('Publishers', 'mybooktable');
		} else {
			$tax = 'mbt_author';
		...
		<div class="mbt-taxonomies-widget-editor">
			...
			<p>
				...
				<select name="<?php echo $this->get_field_name('tax'); ?>" id="<?php echo $this->get_field_id('tax'); ?>" class="widefat">
					...
					<option value="mbt_publisher"<?php selected($tax, 'mbt_publisher'); ?>><?php _e('Publishers', 'mybooktable'); ?></option>
				</select>
			</p>
			<p>
